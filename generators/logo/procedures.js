...
Logo['procedures_defreturn'] = function(block) {
  ...
  const funcName = Logo.variableDB_.getName(
      block.getFieldValue('NAME'), Blockly.PROCEDURE_CATEGORY_NAME);
  let xfix1 = '';
  if (Logo.STATEMENT_PREFIX) {
    xfix1 += Logo.injectId(Logo.STATEMENT_PREFIX,
        block);
  }
  if (Logo.STATEMENT_SUFFIX) {
    xfix1 += Logo.injectId(Logo.STATEMENT_SUFFIX,
        block);
  }
  if (xfix1) {
  ...
  if (Logo.INFINITE_LOOP_TRAP) {
    loopTrap = Logo.prefixLines(
        Logo.injectId(Logo.INFINITE_LOOP_TRAP,
        block), Logo.INDENT);
  }
  const branch = Logo.statementToCode(block, 'STACK');
  let returnValue = Logo.valueToCode(block, 'RETURN',
      Logo.ORDER_NONE) || '';
  let xfix2 = '';
  ...
  for (let i = 0; i < block.arguments_.length; i++) {
    args[i] = Logo.variableDB_.getName(block.arguments_[i],
...
Logo['procedures_defnoreturn'] =
    Logo['procedures_defreturn'];
...
Logo['procedures_callreturn'] = function(block) {
  ...
  const args = [];
  for (let i = 0; i < block.arguments_.length; i++) {
    args[i] = Logo.valueToCode(block, 'ARG' + i,
        Logo.ORDER_NONE) || '[]';
  }
...
Logo['procedures_callnoreturn'] = function(block) {
  ...
  const tuple = Logo['procedures_callreturn'](block);
  return tuple[0] + '\n';
};

Logo['procedures_ifreturn'] = function(block) {
  // Conditionally return value from a procedure.
  const condition = Logo.valueToCode(block, 'CONDITION',
      Logo.ORDER_NONE) || 'false';
  let code = 'if ' + condition + ' [\n';
  if (Logo.STATEMENT_SUFFIX) {
    // Inject any statement suffix here since the regular one at the end
    // will not get executed if the return is triggered.
    code += Logo.prefixLines(
        Logo.injectId(Logo.STATEMENT_SUFFIX, block),
        Logo.INDENT);
  }
  if (block.hasReturnValue_) {
    const value = Logo.valueToCode(block, 'VALUE',
        Logo.ORDER_NONE) || '[]';
