...
Logo['text'] = function(block) {
  // Text value.
  const code = Logo.quote_(block.getFieldValue('TEXT'));
  return [code, Logo.ORDER_ATOMIC];
};

Logo['text_multiline'] = function(block) {
  // Text value.
  let code = Logo.multiline_quote_(block.getFieldValue('TEXT'));
  if (code.includes('\n')) {
      code = '(' + code + ')'
  }
  return [code, Logo.ORDER_ATOMIC];
...
Logo['text_join'] = function(block) {
  ...
  let code;switch (block.itemCount_) {
    ...
    case 2:
      const element0 = Logo.valueToCode(block, 'ADD0',
          Logo.ORDER_ATOMIC) || '"';
      const element1 = Logo.valueToCode(block, 'ADD1',
          Logo.ORDER_NONE) || '"';
...
Logo['text_append'] = function(block) {
  ...
  const value = Logo.valueToCode(block, 'TEXT',
      Logo.ORDER_NONE) || '"';
...
Logo['text_length'] = function(block) {
  ...
  const text = Logo.valueToCode(block, 'VALUE',
      Logo.ORDER_NONE) || '\'\'';
...
Logo['text_charAt'] = function(block) {
  ...
  switch (where) {
    ...
    case 'LAST':
      ...
      return [code, Logo.ORDER_PROCEDURE];
    case 'FROM_START':
      at = Logo.getAdjusted(block, 'AT');
      // Adjust index if using one-based indices.
      code = 'item ' + at + ' ' + text;
      return [code, Logo.ORDER_PROCEDURE];
    case 'FROM_END':
      at = Logo.getAdjusted(block, 'AT', -1, true);
      const procedureName = Logo.provideFunction_(
          'wordLetterFromEnd',
...
// Change capitalization.
  ...
  const text = Logo.valueToCode(block, 'TEXT',
      Logo.ORDER_NONE) || '"';
  if (operator) {
...
Logo['text_print'] = function(block) {
  // Print statement.
  const msg = Logo.valueToCode(block, 'TEXT',
      Logo.ORDER_NONE) || '"';
  return 'pr ' + msg + '\n';
};
...
// Prompt function.
  if (block.getField('TEXT')) {
    ...
    // External message.
    msg = Logo.valueToCode(block, 'TEXT',
        Logo.ORDER_NONE) || '\'\'';
  }
